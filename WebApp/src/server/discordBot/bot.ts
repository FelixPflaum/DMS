...
export class Discordbot {
     ...
     * @param guild
     */
    private async refreshCommandsForGuild(guild: Guild): Promise<void> {
        ...
        try {
            ...
            this.logger.log(
                `Successfully refreshed ${Array.isArray(data) ? data.length : "??"} application commands for guild ${guild.name} (${guild.id}).`
            );
        } catch (error) {
